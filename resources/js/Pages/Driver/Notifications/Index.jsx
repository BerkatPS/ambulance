...
export default function NotificationsIndex({ auth, notifications, unreadCount }) {
    ...
    const handleMarkAsRead = (id) => {
        setProcessing(true);
        post(route('driver.notifications.read', id), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleMarkAllAsRead = () => {
        setProcessing(true);
        post(route('driver.notifications.read.all'), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                setProcessing(true);
                destroy(route('driver.notifications.destroy', id), {
                    preserveScroll: true,
                    onSuccess: () => {
                        Swal.fire(
                            'Terhapus!',
    ...
    return (
        ...
        >
            ...
            <NotificationListener 
                userType="driver"
                channelName={`driver.notifications.${auth.user.id}`}
                user={auth.user}
            />
                ...
                <div className="bg-white sm:rounded-xl shadow-sm px-4 sm:px-6 py-5 border border-slate-100">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div className="flex items-center">
                            <BellAlertIcon className="h-7 w-7 text-primary-600 mr-3" />
                            <div>
                    ...
                    {notifications.data && notifications.data.length > 0 ? (
                        ...
                        <div className="p-12 text-center">
                            <div className="mx-auto w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center mb-4">
                                <BellAlertIcon className="h-10 w-10 text-slate-400" />
                            </div>
...
function getTypeIcon(type) {
    if (!type) return <BellAlertIcon className="h-6 w-6" />;
    ...
    if (type && type.includes && type.includes('\\Notifications\\')) {
        ...
        } else if (type.includes('Driver') || type.includes('Profile')) {
    ...
    // Legacy/fallback handling
    if (!type || typeof type !== 'string') return <BellAlertIcon className="h-6 w-6" />;
